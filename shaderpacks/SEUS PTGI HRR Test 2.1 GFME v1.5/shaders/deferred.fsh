               ...
               if(c(T.QbpObHBdUl,j,P,G,K))
                 {
                   ...
                   specularData=texture2DLod(depthtex0,coord,0);
       ...
       if(j<1.f||j>254.f)
       ...
       else
         {
           ...
           if(specularData[SPEC_CHANNEL_EMISSIVE]>0.)
             {
               y+=.1*M*GI_LIGHT_BLOCK_INTENSITY*specularData[SPEC_CHANNEL_EMISSIVE];
             }
           else
           #endif
             {
               ...
               if(lightFix>.01)
                 {
                   ...
                   float x=e(specularData[SPEC_CHANNEL_SMOOTHNESS],specularData[SPEC_CHANNEL_METALNESS]);
                   float z=1.+(-x+25.*x*max((dot(worldLightVector,reflect(P.direction,K))-x*.9)*10.,0.))*(specularData[SPEC_CHANNEL_METALNESS]*.98+.02);
                   y+=N*z;
                 }
             }
         }
