...
vec2 Texcoord;
 ...
 vec3 e(vec3 v,vec3 i,vec3 y)
 {
   if(wetness>.99)
     return vec3(0.);
   v+=1.;
   v-=FractedCameraPosition;
   vec3 s=n(v);
   float depth=pow(length(v),.7)*3e-5+3e-5;
   float shadowStrength=shadow2DLod(shadowtex0,vec3(s.xy,s.z-depth),0).x;
   vec3 c=vec3(shadowStrength);
   if(shadowStrength<.1)
     {
       c*=saturate(dot(i,y));
   ...
   float stainedGlassShadow=shadow2DLod(shadowtex0,vec3(s.xy-vec2(0.5,0.0),s.z-depth),0).x;
   if(stainedGlassShadow<.9)
     {
       vec3 stainedGlassColor=texture2DLod(shadowcolor,vec2(s.xy-vec2(0.5,0.0)),3).rgb;
       stainedGlassColor*=stainedGlassColor;
       c=mix(c*stainedGlassColor,c,stainedGlassShadow);
     }
   c*=saturate(dot(i,y));
   float t=shadow2DLod(shadowtex0,vec3(s.xy-vec2(0.,.5),s.z+1e-06),2).x;
   if(t<1)
     {
       vec4 d=texture2DLod(shadowcolor1,s.xy-vec2(0.,.5),4);
       float w=abs(d.x*256.-(v.y+cameraPosition.y)),h=GetCausticsComposite(v,i,w);
       c=mix(c*h,c,t);
     }
   c=TintUnderwaterDepth(c);
   #ifdef CLOUD_SHADOW
   ...
   for(int Q=0;Q<1;Q++)
     {
       ...
       if(abs(Y.w-0.5)>0.499)
         {
           ...
           #ifdef CLOUDS_IN_GI
           CloudPlane(A,s,-F.direction,worldLightVector,worldSunVector,colorSunlight,colorSkyUp,L,timeMidnight,true);
           #endif
       ...
       irradiance/=weight;
       N+=irradiance*2.4*k;
       N+=e(M+F.direction*J-1.,worldLightVector,u)*k*2.4*colorSunlight*l;
